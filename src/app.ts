...
function Validates(validation: string) {
  return function(target: any, propertyName: string) {
    const className = target.constructor.name;
    let validators = registeredValidators[className] || {};
    let propValidators = validators[propertyName] || [];
    registeredValidators[className] = { ...validators, [propertyName]: [...propValidators, validation] };
  }
}
...
class Course {
  @Validates('required') title: string;
  @Validates('positive') price: number;
